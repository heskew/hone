...
# Empty /data for the database. The runtime image has no shell, so the
# directory is created here and copied in. uid 65532 can write it.
FROM debian:trixie-slim AS data-dir
RUN mkdir -p /data && chown 65532:65532 /data && chmod 755 /data
...
COPY --from=data-dir --chown=65532:65532 --chmod=755 /data /data
EXPOSE 3000
ENTRYPOINT ["/app/hone"]
