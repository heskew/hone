...
- MCP OAuth resource server (RFC 9728 metadata, `WWW-Authenticate`, RFC 8707 audience-bound JWTs; rejected on `/api`)
- Opaque MCP-only keys as a local fallback (`HONE_MCP_KEYS`; accepted on `/mcp`, rejected on `/api`)
