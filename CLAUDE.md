...
- **hone-server**: Axum REST API
  ...
  - `mcp/` - MCP server for LLM tool access (enable with `--mcp-port 3001`; OAuth 2.1 resource server: RFC 9728 metadata + RFC 8707 audience-bound JWTs; opaque `HONE_MCP_KEYS` still `/mcp`-only; `HONE_API_KEYS` still work on `/mcp`)
...
3. **Local AI only** - Ollama and OpenAI-compatible servers. No cloud APIs. `OLLAMA_HOST` and `ANTHROPIC_COMPATIBLE_HOST` are refused when they are public unless `HONE_ALLOW_REMOTE_AI=1`. AI HTTP clients do not follow redirects. A local-looking hostname that resolves outside loopback, RFC1918, or IPv6 unique-local is refused.
